...
// specifying the loader in ComposeOptions.
//
...
func newResolverContext(options *specs.ExpandOptions) *resolverContext {
	composeOptions := optionsOrDefault(options)
	...
	if composeOptions.PathLoader == nil {
		...
		loader = composeOptions.PathLoader
	}

	return &resolverContext{
		circulars: make(map[string]bool),
		basePath:  composeOptions.RelativeBase, // keep the root base path in context
		loadDoc:   loader,
	}
}
...
type schemaLoader struct {
	root    interface{}
	options *specs.ExpandOptions
	cache   specs.ResolutionCache
	context *resolverContext
}

func (r *schemaLoader) transitiveResolver(basePath string, ref specs.Ref) *schemaLoader {
	if ref.IsRoot() || ref.HasFragmentOnly {
		return r
	}

	baseRef := specs.MustCreateRef(basePath)
...
func (r *schemaLoader) resolveRef(ref *specs.Ref, target interface{}, basePath string) error {
	...
	if (ref.IsRoot() || ref.HasFragmentOnly) && root == nil && basePath != "" {
		if baseRef, erb := specs.NewRef(basePath); erb == nil {
...
func (r *schemaLoader) isCircular(ref *specs.Ref, basePath string, parentRefs ...string) (foundCycle bool) {
...
func (r *schemaLoader) Resolve(ref *specs.Ref, target interface{}, basePath string) error {
...
func defaultSchemaLoader(
	root interface{},
	composeOptions *specs.ExpandOptions,
	cache specs.ResolutionCache,
	context *resolverContext) *schemaLoader {

	if composeOptions == nil {
		composeOptions = &specs.ExpandOptions{}
	}

	cache = cacheOrDefault(cache)

	if composeOptions.RelativeBase == "" {
		...
		composeOptions.RelativeBase = baseForRoot(root, cache)
	}

	if context == nil {
		context = newResolverContext(composeOptions)
	}

...
// normalizeURI ensures that all $ref paths used internally by the composeer are canonicalized.
//
...
func normalizeURI(refPath, base string) string {
	...
	r := specs.MustCreateRef(refURL.String())
	if r.IsCanonical() {
...
func rebase(ref *specs.Ref, v *url.URL, notEqual bool) (specs.Ref, bool) {
	var newBase url.URL
	...
	return specs.MustCreateRef(newBase.String()), true
}

// normalizeRef canonicalize a Ref, using a canonical relativeBase as its absolute anchor
func normalizeRef(ref *specs.Ref, relativeBase string) *specs.Ref {
	r := specs.MustCreateRef(normalizeURI(ref.String(), relativeBase))
	return &r
}
